...
module.exports.total_quiz_score = async function (req, res) {
    try {
        student_id = req["student_id"]
        var sql_query1 = 'select sum(score) AS total_score from answer where student_id=?'
        var values = [student_id];

        var results = await execute_query(sql_query1, values)
        responses.sendResponse(res, `Congratulation!! Your Total score is ${results[0].total_score}`, constants.STATUS_CODES.SUCCESS)
